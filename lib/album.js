...
function getThumbPath(albumPath, pathFromReq) {
  const cached = cache.get(albumPath);
  if (cached) {
    return cached;
  }
  const files = fs.readdirSync(albumPath);
  const photos = _getPhotos(files, albumPath, pathFromReq);
  if (photos.length > 0) {
    cache.put(albumPath, photos[0].src);
    return photos[0].src;
  }
  const albums = _getAlbums(files, albumPath, pathFromReq);
  const thumbPath = albums.reduce((acc, album) => acc || album.thumbPath, null);
  cache.put(albumPath, thumbPath);
  return thumbPath;
}
...
function _thumbnail(albumPath, pathFromRequest, cb){
  ...
  fs.readdir(albumPath, function (err, files) {
    ...
    if (photos.length > 0){
      ...
      var firstPhoto = photos[0].path;
      firstPhoto = path.join(config.staticFiles, firstPhoto);
      return cb(null, firstPhoto)
    } else {
      function new_cb(err, photo) {
        if (err) {
          if (albums.length === 0) {
            return cb('No suitable thumbnail found');  
          } else {
            const firstAlbum = albums.shift().name;
            return _thumbnail(
              path.join(albumPath, firstAlbum), path.join(pathFromRequest, firstAlbum), new_cb);
          }
        }
